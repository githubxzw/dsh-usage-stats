...
/**
 * Local patch (persistence-contract-2026-09): build a `readEvents(id, fromSeq)`
 * adapter over either sessionPersistence generation. The legacy contract
 * exposes `readFrom(id, fromSeq)` resolving `{ events }`; the redesigned
 * contract (DSH 2026-09) exposes `open(id, 'read')` handles whose
 * `read(offset)` returns events with `seq >= offset` — the same inclusive
 * semantics the fold assumes. Returns `null` when neither shape exists.
 */
function createPersistenceEventReader(persistence) {
	if (typeof persistence.readFrom === "function") {
		return async (id, fromSeq) => (await persistence.readFrom(id, fromSeq)).events;
	}
	if (typeof persistence.open === "function") {
		return async (id, fromSeq) => {
			...
			try {
				...
				try {
					...
					// close() is idempotent and uncancellable; a failed close on an
					// already-broken read must not mask the original error.
				}
			}
		};
	}
 ...
 * unchanged (`listSnapshots` on legacy builds, `list()` snapshots on the
 * redesigned contract, falling back to always reading the delta); when the
 * revision changes, the new events are verified
...
export async function collectUsage(ctx, config = { monitors: {}, budgets: validateBudgetConfig() }) {
	return withLock(async () => {
		...
		if (persistence !== void 0) {
			// Local patch (persistence-contract-2026-09): DSH redesigned the
			// sessionPersistence service. Legacy builds expose `listSnapshots()`
			// returning {header, revision} entries plus `readFrom(id, seq)`;
			// redesigned builds expose `list()` returning the SAME {header,
			// revision} snapshot objects (plus optional eventCount/sizeBytes)
			// and read events through `open(id, 'read')` handles. Normalize
			// both generations here so the fold below stays contract-agnostic.
			const readEvents = createPersistenceEventReader(persistence);
			if (readEvents === null) {
				ctx.logger.warn("usage-stats: sessionPersistence exposes neither readFrom() nor open(); persisted-session usage is unavailable");
			}
			let metas = null;
			const revisionOf = new Map();
			if (typeof persistence.listSnapshots === "function") {
				// Legacy generation: snapshots already carry {header, revision}.
				let snapshots = null;
				try {
				...
				if (snapshots !== null) {
					...
					for (const entry of snapshots) revisionOf.set(entry.header.id, entry.revision);
				}
			}
			if (metas === null) {
				// Current generation: list() returns {header, revision} snapshot
				// objects; the legacy list() returned bare headers (no revision
				// available → every log is re-read, same as the old fallback).
				const listed = await persistence.list();
				metas = listed.map((entry) => entry !== null && typeof entry === "object" && entry.header !== void 0 ? entry.header : entry);
				for (const entry of listed) {
					if (entry !== null && typeof entry === "object" && entry.header?.id !== void 0 && entry.revision !== void 0) {
						revisionOf.set(entry.header.id, entry.revision);
					}
				}
			}
			for (const meta of metas) {
				...
				if (changed && readEvents !== null) {
					try {
						const wasPersisted = state.kind === "persisted";
						const fromSeq = wasPersisted ? state.consumed : 0;
						const events = await readEvents(meta.id, fromSeq);
						if (!wasPersisted) {
							resetUsageState(state);
						}
						const fresh = wasPersisted ? events.filter((event) => event.seq > (state.consumed ?? 0)) : events;
						// readFrom / handle.read are inclusive: an unchanged log returns
						// exactly the already-folded cursor event (seq === consumed). An
						// empty fresh slice with the cursor still present is therefore
						// NOT a rewrite — only a log that no longer contains the cursor
						...
						if (!contiguous && state.consumed > 0) {
							...
							resetUsageState(state);
							const allEvents = await readEvents(meta.id, 0);
