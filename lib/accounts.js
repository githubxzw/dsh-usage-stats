...
function customSubscription(spec, body, now) {
	const extract = spec.monitor.extract;
	const root = jsonPointer(body, extract.root ?? "");
	const itemsRaw = mapped(root, extract.items);
	// Local patch: accept a single window object (e.g. New-API
	// /api/user/self/daily-limit) in addition to the array shape.
	if (itemsRaw === null || itemsRaw === void 0 || typeof itemsRaw !== "object") throw statusError("invalid-response", "custom response items must be an array or an object");
	...
	for (const item of items) {
		...
		windows.push({
			// Local patch: kindValue mirrors the extract.currencyValue literal fallback.
			kind: nonEmptyString(mapped(item, extract.kind)) ?? nonEmptyString(extract.kindValue) ?? "quota",
			usedPercent,
			remainingPercent,
